Page({
  ...
  async loadList() {
    try {
      ...
      const res = await db.collection('demands').where({ type: '农践帮' }).orderBy('createdAt', 'desc').get();
      const list = (res.data || []).map(x => ({ ...x, time: x.time || this.formatRelative(x.createdAt) }));
      this.setData({ list });
    } catch (e) {
      const app = getApp();
      this.setData({ list: app.globalData.demands.njb || [] });
    }
  },
  ...
  goCreate() { wx.navigateTo({ url: '/pages/demand/create?type=农践帮' }); },
